...
#include "LtePdcpRrcUeAutoD2D.h"
#include "L3AddressResolver.h"
#include "D2DModeSwitchNotification_m.h"

Define_Module(LtePdcpRrcUeAutoD2D);

/*
 * Upper Layer handlers
 */
void LtePdcpRrcUeAutoD2D::fromDataPort(cPacket *pkt)
{
    ...
    // else the direction will be selected according to the current status of the UE, i.e. AutoD2D or UL
...
//    {
        ...
        // using AutoD2D, set AutoD2D direction. Otherwise, set UL direction
...
//    }
    ...
    // Cid Request
    EV << NOW << " LtePdcpRrcUeAutoD2D : Received CID request for Traffic [ " << "Source: "
        ...
        lteInfo->getSrcPort(), lteInfo->getDstPort(), lteInfo->getDirection())) == 0xFFFF)
    {
        ...
        EV << "LtePdcpRrcUeAutoD2D : Connection not found, new CID created with LCID " << mylcid << "\n";

        ht_->create_entry(lteInfo->getSrcAddr(), lteInfo->getDstAddr(),
            lteInfo->getSrcPort(), lteInfo->getDstPort(), lteInfo->getDirection(), mylcid);
    }

    EV << "LtePdcpRrcUeAutoD2D : Assigned Lcid: " << mylcid << "\n";
    EV << "LtePdcpRrcUeAutoD2D : Assigned Node ID: " << nodeId_ << "\n";
    ...
    if (lteInfo->getDirection() == D2D)
        ...
        lteInfo->setDestId(nodeId_);             // destId is meaningless for multicast AutoD2D (we use the id of the source for statistic purposes at lower levels)
    else // UL
...
void LtePdcpRrcUeAutoD2D::initialize(int stage)
{
    EV << "LtePdcpRrcUeAutoD2D::initialize() - stage " << stage << endl;
    if (stage == inet::INITSTAGE_LOCAL)
        LtePdcpRrcBase::initialize();
    if (stage == INITSTAGE_NETWORK_LAYER_3+1)
    {
        // inform the Binder about the AutoD2D capabilities of this node
        // i.e. the (possibly) AutoD2D peering UEs
...
void LtePdcpRrcUeAutoD2D::handleMessage(cMessage* msg)
{
...
void LtePdcpRrcUeAutoD2D::pdcpHandleD2DModeSwitch(MacNodeId peerId, LteD2DMode newMode)
{
    EV << NOW << " LtePdcpRrcUeAutoD2D::pdcpHandleAutoD2DModeSwitch - peering with UE " << peerId << " set to " << d2dModeToA(newMode) << endl;
