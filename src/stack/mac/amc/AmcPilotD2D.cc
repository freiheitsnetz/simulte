...
const UserTxParams& AmcPilotD2D::computeTxParams(MacNodeId id, const Direction dir)
{
    ...
    {
        //Do what the eNB does
        computeTxParamsUnassisted(id, dir);
    }
...
const UserTxParams& AmcPilotD2D::computeTxParamsUnassisted(MacNodeId id, const Direction dir)
{
    EV << NOW << " AmcPilotD2D" << getName() << "::computeTxParamsUnassisted for UE " << id << ", direction " << dirToA(dir) << endl;

    // Check if user transmission parameters have been already allocated
    if(amc_->existTxParams(id, dir))
    {
        EV << NOW << " AmcPilotD2D" << getName() << "::computeTxParamsUnassisted The Information for this user have been already assigned \n";
        return amc_->getTxParams(id, dir);
    }
    ...
    sfb.print(0,id,dir,txMode,"AmcPilotD2D::computeTxParamsUnassisted");
    ...
    if(mode_ == MAX_CQI)
    {
        ...
        if (usableB == NULL || usableB->empty())
        {
            ...
            EV << NOW <<" AmcPilotD2D::computeTxParamsUnassisted - no UsableBand available for this user." << endl;
        }
        else
        {
            ...
            EV << NOW <<" AmcPilotD2D::computeTxParamsUnassisted - UsableBand of size " << usableB->size() << " available for this user" << endl;
        }
    }
    ...
    // DEBUG
    EV << NOW << " AmcPilot" << getName() << "::computeTxParamsUnassisted NEW values assigned! - CQI =" << chosenCqi << "\n";
    info.print("AmcPilotD2D::computeTxParamsUnassisted");
