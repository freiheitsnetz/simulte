...
// Same as LCG Scheduler Realistic
...
//    TODO                    elem->bucket_ = 100.0 /* TODO desc->parameters_.bucket_*/;
                    ...
                    EV << NOW << " LteUnassistedD2DSchedulingAgent::scheduleData - Node " << ueScheduler_->ueMac_->getMacNodeId() << ",  SDU of size " << elem->sentData_ << " selected for transmission" << endl;
                    EV << NOW << " LteUnassistedD2DSchedulingAgent::scheduleData - Node " << ueScheduler_->ueMac_->getMacNodeId() << ", remaining grant: " << availableBytes << " bytes" << endl;
                    EV << NOW << " LteUnassistedD2DSchedulingAgent::scheduleData - Node " << ueScheduler_->ueMac_->getMacNodeId() << " buffer Size: " << toServe << " bytes" << endl;
                }
                else
                {
