...
void LteMaxCiAutoD2D::prepareSchedule()
{
    EV << NOW << " LteMaxCIAutoD2D::schedule " << ueAutoD2DScheduler_->mac_->getMacNodeId() << endl;
    ...
    for ( ActiveSet::iterator it1 = activeConnectionTempSet_.begin ();it1 != activeConnectionTempSet_.end (); )
    {
        ...
        const UserTxParams& info = ueAutoD2DScheduler_->mac_->getAmc()->computeTxParams(nodeId,dir);
        ...
        continue;
        //no more free cw
        if (ueAutoD2DScheduler_->allocatedCws(nodeId)==codeword)
        continue;
        ...
        for (;antennaIt!=antennaEt;++antennaIt)
        {
            ...
            for (;it!=et;++it)
            {
                availableBlocks += ueAutoD2DScheduler_->readAvailableRbs(nodeId,*antennaIt,*it);
                availableBytes += ueAutoD2DScheduler_->mac_->getAmc()->computeBytesOnNRbs(nodeId,*it, availableBlocks, dir);
            }
        }
        ...
        EV << NOW << " LteMaxCIAutoD2D::schedule computed for cid " << cid << " score of " << desc.score_ << endl;
    }
    ...
    while ( ! score.empty () )
    {
        ...
        EV << NOW << " LteMaxCIAutoD2D::schedule scheduling connection " << current.x_ << " with score of " << current.score_ << endl;
        ...
        EV << NOW << "LteMaxCIAutoD2D::schedule granted " << granted << " bytes to connection " << current.x_ << endl;
        ...
        {
            score.pop ();
            EV << NOW << "LteMaxCIAutoD2D::schedule  connection " << current.x_ << " was found ineligible" << endl;
        }

        // Set the connection as inactive if indicated by the grant ().
        if ( ! active )
        {
            EV << NOW << "LteMaxCIAutoD2D::schedule scheduling connection " << current.x_ << " set to inactive " << endl;
...
void LteMaxCiAutoD2D::commitSchedule()
{
    activeConnectionSet_ = activeConnectionTempSet_;
}

void LteMaxCiAutoD2D::updateSchedulingInfo()
{
}

void LteMaxCiAutoD2D::notifyActiveConnection(MacCid cid)
{
    EV << NOW << "LteMaxCIAutoD2D::notify CID notified " << cid << endl;
    activeConnectionSet_.insert(cid);
}

void LteMaxCiAutoD2D::removeActiveConnection(MacCid cid)
{
    EV << NOW << "LteMaxCIAutoD2D::remove CID removed " << cid << endl;
    activeConnectionSet_.erase(cid);
}
