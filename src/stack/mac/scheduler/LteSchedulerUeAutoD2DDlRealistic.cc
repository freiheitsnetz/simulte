...
LteSchedulerUeAutoD2DDlRealistic::LteSchedulerUeAutoD2DDlRealistic() {
 ...
 */
unsigned int LteSchedulerUeAutoD2DDlRealistic::scheduleGrant(MacCid cid, unsigned int bytes,
        ...
        Remote antenna, bool limitBl)
{
    ...
    EV << "LteSchedulerUeAutoD2DDlRealistic::grant(" << cid << "," << bytes << "," << terminate << "," << active << "," << eligible << "," << bands_msg << "," << dasToA(antenna) << ")" << endl;
    ...
    EV << "LteSchedulerUeAutoD2DDlRealistic::grant --------------------::[ START GRANT ]::--------------------" << endl;
    EV << "LteSchedulerUeAutoD2DDlRealistic::grant Cell: " << mac_->getMacCellId() << endl;
    EV << "LteSchedulerUeAutoD2DDlRealistic::grant CID: " << cid << "(UE: " << nodeId << ", Flow: " << flowId << ") current Antenna [" << dasToA(antenna) << "]" << endl;
    ...
    if (mac_->muMimo() && (txParams.readTxMode() == MULTI_USER))
    {
        ...
        if (peer != nodeId)
        {
            ...
            {
                EV << "LteSchedulerUeAutoD2DDlRealistic::grant MU-MIMO pairing established: main user [" << nodeId << "], paired user [" << peer << "]" << endl;
            }
            else
            {
                EV << "LteSchedulerUeAutoD2DDlRealistic::grant MU-MIMO pairing already exists between users [" << nodeId << "] and [" << peer << "]" << endl;
            }
        }
        else
        {
            EV << "LteSchedulerUeAutoD2DDlRealistic::grant no MU-MIMO pairing available for user [" << nodeId << "]" << endl;
        }
    }
        ...
        (txParams.readTxMode() != MULTI_USER || plane != MU_MIMO_PLANE)))
    {
        ...
        EV << "LteSchedulerUeAutoD2DDlRealistic::grant Space ended, no schedulation." << endl;
        return 0;
    }
    ...
    // DEBUG OUTPUT
    if (limitBl)
        EV << "LteSchedulerUeAutoD2DDlRealistic::grant blocks: " << bytes << endl;
        else
        EV << "LteSchedulerUeAutoD2DDlRealistic::grant Bytes: " << bytes << endl;
    EV << "LteSchedulerUeAutoD2DDlRealistic::grant Bands: {";
    ...
    EV << "LteSchedulerUeAutoD2DDlRealistic::grant TxMode: " << txModeToA(txParams.readTxMode()) << endl;
    EV << "LteSchedulerUeAutoD2DDlRealistic::grant Available codewords: " << numCodewords << endl;
    ...
    if (!checkEligibility(nodeId, cw) || cw >= numCodewords)
    {
        ...
        EV << "LteSchedulerUeAutoD2DDlRealistic::grant @@@@@ CODEWORD " << cw << " @@@@@" << endl;
        EV << "LteSchedulerUeAutoD2DDlRealistic::grant Total allocation: " << totalAllocatedBytes << "bytes" << endl;
        EV << "LteSchedulerUeAutoD2DDlRealistic::grant NOT ELIGIBLE!!!" << endl;
        EV << "LteSchedulerUeAutoD2DDlRealistic::grant --------------------::[  END GRANT  ]::--------------------" << endl;
    ...
    for (; cw < numCodewords; ++cw)
    {
        ...
        if (queueLength == 0)
        {
            ...
            EV << "LteSchedulerUeAutoD2DDlRealistic::grant scheduled connection is no more active . Exiting grant " << endl;
            stop = true;
            break;
        }
        else
        ...
        EV << "LteSchedulerUeAutoD2DDlRealistic::grant bytes to be allocated: " << toBook << endl;
        ...
        for (unsigned int i = 0; i < size; ++i)
        {
            ...
            EV << "LteSchedulerUeAutoD2DDlRealistic::grant --- BAND " << b << " LIMIT " << limit << "---" << endl;

            // if the limit flag is set to skip, jump off
            if (limit == -2)
            {
                EV << "LteSchedulerUeAutoD2DDlRealistic::grant skipping logical band according to limit value" << endl;
                continue;
            }
            ...
            {
                EV << "LteSchedulerUeAutoD2DDlRealistic::grant Band " << b << "will be skipped since it has no space left." << endl;
            ...
            if (!limitBl)
            {
                ...
                if (limit >= 0 && limit < (int) bandAvailableBytes)
                {
                    ...
                    EV << "LteSchedulerUeAutoD2DDlRealistic::grant Band space limited to " << bandAvailableBytes << " bytes according to limit cap" << endl;
                }
            }
            else
            {
                ...
                if(limit >= 0 && limit < (int) bandAvailableBlocks)
                {
                    ...
                    EV << "LteSchedulerUeAutoD2DDlRealistic::grant Band space limited to " << bandAvailableBlocks << " blocks according to limit cap" << endl;
                }
            }

            EV << "LteSchedulerUeAutoD2DDlRealistic::grant Available Bytes: " << bandAvailableBytes << " available blocks " << bandAvailableBlocks << endl;
            ...
            EV << "LteSchedulerUeAutoD2DDlRealistic::grant Booking band available blocks" << (bandAvailableBlocks-blocksAdded) << " [" << bandAvailableBytes << " bytes] for future use, going to next band" << endl;
            // enable booking  here
        ...
        for (; li != le; ++li)
        {
            ...
            EV << "LteSchedulerUeAutoD2DDlRealistic::grant Band " << li->b_ << " can contribute with " << li->bytes_ << " of booked resources " << endl;
        }

        // get resources to allocate
        unsigned int toServe = queueLength - toBook;

        EV << "LteSchedulerUeAutoD2DDlRealistic::grant servicing " << toServe << " bytes with " << totalBooked << " booked bytes " << endl;
        ...
        if (totalBooked>0)
        {
            ...
            EV << "LteSchedulerUeAutoD2DDlRealistic::grant Making use of booked resources [" << totalBooked << "] for inter-band data allocation" << endl;
            ...
            while ((li!=le) && (bookedUsed<=toServe))
            {
                ...
                EV << "LteSchedulerUeAutoD2DDlRealistic::grant allocating " << uBytes << " prev. booked bytes on band " << (unsigned short)u << endl;
                ...
                else
                {
                    ...
                    EV << "LteSchedulerUeAutoD2DDlRealistic::grant band " << (unsigned short)u << " depleted all its booked resources " << endl;
                }
            }
            vQueueItemCounter++;
        }
        ...
        EV << "LteSchedulerUeAutoD2DDlRealistic::grant Codeword allocation: " << cwAllocatedBytes << "bytes" << endl;
        ...
        if (cwAllocatedBytes > 0)
        {
            ...
            EV << "LteSchedulerUeAutoD2DDlRealistic::grant CODEWORD IS NOW BUSY: GO TO NEXT CODEWORD." << endl;
        ...
        {
            EV << "LteSchedulerUeAutoD2DDlRealistic::grant CODEWORD IS FREE: NO ALLOCATION IS POSSIBLE IN NEXT CODEWORD." << endl;
            eligible = false;
            stop = true;
        }
        if (stop)
    ...
    EV << "LteSchedulerUeAutoD2DDlRealistic::grant Total allocation: " << totalAllocatedBytes << " bytes, " << totalAllocatedBlocks << " blocks" << endl;
    EV << "LteSchedulerUeAutoD2DDlRealistic::grant --------------------::[  END GRANT  ]::--------------------" << endl;
