...
// TODO
bool
LteSchedulerUeAutoD2DUl::checkEligibility(MacNodeId id, Codeword& cw)
{
...
void LteSchedulerUeAutoD2DUl::updateHarqDescs()
{
    EV << NOW << "LteSchedulerUeAutoD2DUl::updateHarqDescs  cell " << mac_->getMacCellId() << endl;
    ...
    for (it=harqRxBuffers_->begin();it!=harqRxBuffers_->end();++it)
    {
        ...
        {
            EV << NOW << "LteSchedulerUeAutoD2DUl::updateHarqDescs UE " << it->first << " OLD Current Process is  " << (unsigned int)currentStatus->second << endl;
            // updating current acid id
            currentStatus->second = (currentStatus->second +1 ) % (it->second->getProcesses());

            EV << NOW << "LteSchedulerUeAutoD2DUl::updateHarqDescs UE " << it->first << "NEW Current Process is " << (unsigned int)currentStatus->second << "(total harq processes " << it->second->getProcesses() << ")" << endl;
        }
        else
        {
            EV << NOW << "LteSchedulerUeAutoD2DUl::updateHarqDescs UE " << it->first << " initialized the H-ARQ status " << endl;
            harqStatus_[it->first]=0;
        }
    }
}

bool LteSchedulerUeAutoD2DUl::racschedule()
{
    EV << NOW << " LteSchedulerUeAutoD2DUl::racschedule --------------------::[ START RAC-SCHEDULE ]::--------------------" << endl;
    EV << NOW << " LteSchedulerUeAutoD2DUl::racschedule eNodeB: " << mac_->getMacCellId() << endl;
    EV << NOW << " LteSchedulerUeAutoD2DUl::racschedule Direction: " << (direction_ == UL ? "UL" : "DL") << endl;
    ...
    for (;it!=et;++it)
    {
        ...
        EV << NOW << " LteSchedulerUeAutoD2DUl::racschedule handling RAC for node " << nodeId << endl;
        ...
        for (Band b=0;b<numBands;++b)
        {
            if ( allocator_->availableBlocks(nodeId,MACRO,b) >0)
            {
                ...
                if (bytes > 0)
                {
                    ...
                    EV << NOW << "LteSchedulerUeAutoD2DUl::racschedule UE: " << nodeId << "Handled RAC on band: " << b << endl;
    ...
    EV << NOW << " LteSchedulerUeAutoD2DUl::racschedule --------------------::[  END RAC-SCHEDULE  ]::--------------------" << endl;
...
bool
LteSchedulerUeAutoD2DUl::rtxschedule()
{
    ...
    try
    {
        EV << NOW << " LteSchedulerUeAutoD2DUl::rtxschedule --------------------::[ START RTX-SCHEDULE ]::--------------------" << endl;
        EV << NOW << " LteSchedulerUeAutoD2DUl::rtxschedule eNodeB: " << mac_->getMacCellId() << endl;
        EV << NOW << " LteSchedulerUeAutoD2DUl::rtxschedule Direction: " << (direction_ == UL ? "UL" : "DL") << endl;
        ...
        for(; it != et; ++it)
        {
            ...
            EV << NOW << "LteSchedulerUeAutoD2DUl::rtxschedule UE: " << nodeId << "Acid: " << (unsigned int)currentAcid << endl;
            ...
            for(Codeword cw = 0; (cw < MAX_CODEWORDS) && (codewords>0); ++cw)
            {
                ...
                rtxBytes=LteSchedulerUeAutoD2DUl::schedulePerAcidRtx(nodeId, cw,currentAcid);
                if (rtxBytes>0)
                {
                    --codewords;
            ...
            EV << NOW << "LteSchedulerUeAutoD2DUl::rtxschedule user " << nodeId << " allocated bytes : " << allocatedBytes << endl;
        }
        ...
        if (mac_->isD2DCapable())
        {
            ...
            if (strcmp(mac_->getClassName(), "LteMacUeAutoD2D") == 0)
                harqRxBuffersD2DMirror = check_and_cast<LteMacUeAutoD2D*>(mac_)->getRxHarqBufferMirror();
            else if (strcmp(mac_->getClassName(), "LteMacUeAutoD2DRealistic") == 0)
                harqRxBuffersD2DMirror = check_and_cast<LteMacUeAutoD2DRealistic*>(mac_)->getRxHarqBufferMirror();
            else
                throw cRuntimeError("LteSchedulerUeAutoD2DUl::rtxschedule - unrecognized MAC type %s", mac_->getClassName());
            ...
            for(; it_d2d != et_d2d; ++it_d2d)
            {
                ...
                EV << NOW << " LteSchedulerUeAutoD2DUl::rtxschedule - D2D UE: " << senderId << " Acid: " << (unsigned int)currentAcid << endl;
                ...
                for(Codeword cw = 0; (cw < MAX_CODEWORDS) && (codewords>0); ++cw)
                {
                    ...
                    // perform a retransmission on available codewords for the selected acid
                    rtxBytes = LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D(destId, senderId, cw, currentAcid);
                    if (rtxBytes>0)
                    {
                        --codewords;
                        allocatedBytes+=rtxBytes;
                ...
                EV << NOW << " LteSchedulerUeAutoD2DUl::rtxschedule - D2D UE: " << senderId << " allocated bytes : " << allocatedBytes << endl;
        ...
        EV << NOW << " LteSchedulerUeAutoD2DUl::rtxschedule residual OFDM Space: " << availableBlocks << endl;

        EV << NOW << " LteSchedulerUeAutoD2DUl::rtxschedule --------------------::[  END RTX-SCHEDULE  ]::--------------------" << endl;

        return (availableBlocks == 0);
    }
    catch(std::exception& e)
    {
        throw cRuntimeError("Exception in LteSchedulerUeAutoD2DUl::rtxschedule(): %s", e.what());
    }
    return 0;
}

unsigned int
LteSchedulerUeAutoD2DUl::schedulePerAcidRtx(MacNodeId nodeId, Codeword cw, unsigned char acid,
    std::vector<BandLimit>* bandLim, Remote antenna, bool limitBl)
{
    try
    {
        ...
        EV << NOW << "LteSchedulerUeAutoD2DUl::rtxAcid - Node[" << mac_->getMacNodeId() << ", User[" << nodeId << ", Codeword[ " << cw << "], ACID[" << (unsigned int)acid << "] " << endl;
...
//            EV << NOW << " LteSchedulerUeAutoD2DUl::rtxAcid User is on ACID " << (unsigned int)currentAcid << " while requested one is " << (unsigned int)acid << ". No RTX scheduled. " << endl;
//            return 0;
//        }
        ...
        if (currentProcess->getUnitStatus(cw) != RXHARQ_PDU_CORRUPTED)
        {
            ...
            EV << NOW << " LteSchedulerUeAutoD2DUl::rtxAcid User is on ACID " << (unsigned int)currentAcid << " HARQ process is IDLE. No RTX scheduled ." << endl;
            delete(bandLim);
            return 0;
        }
...
//                    ((allocatedCw == MAX_CODEWORDS) ? availableBytes(nodeId,antenna, b, cw) : mac_->getAmc()->blocks2bytes(nodeId, b, cw, allocator_->getBlocks(antenna,b,nodeId) , direction_));    // available space
            ...
            EV << NOW << " LteSchedulerUeAutoD2DUl::rtxAcid BAND " << b << endl;
            EV << NOW << " LteSchedulerUeAutoD2DUl::rtxAcid total bytes:" << bytes << " still to serve: " << toServe << " bytes" << endl;
            EV << NOW << " LteSchedulerUeAutoD2DUl::rtxAcid Available: " << bandAvailableBytes << " bytes" << endl;
        ...
        if (toServe > 0)
        {
            ...
            EV << NOW << " LteSchedulerUeAutoD2DUl::rtxAcid Unavailable space for serving node " << nodeId << " ,HARQ Process " << (unsigned int)currentAcid << " on codeword " << cw << endl;
            delete(bandLim);
            return 0;
        }
        else
        {
            ...
            for(unsigned int i = 0; i < size; ++i)
            {
                ...
                {
                    EV << NOW << " LteSchedulerUeAutoD2DUl::rtxAcid - adding " << assignedBlocks.at(i) << " to band " << i << endl;
            ...
            EV << NOW << " LteSchedulerUeAutoD2DUl::rtxAcid HARQ Process " << (unsigned int)currentAcid << " : " << bytes << " bytes served! " << endl;
    ...
    {
        throw cRuntimeError("Exception in LteSchedulerUeAutoD2DUl::rtxAcid(): %s", e.what());
    }
    delete(bandLim);
    return 0;
}
...
unsigned int
LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D(MacNodeId destId,MacNodeId senderId, Codeword cw, unsigned char acid,
    std::vector<BandLimit>* bandLim, Remote antenna, bool limitBl)
{
    Direction dir = D2D;
    try
    {
        ...
        EV << NOW << "LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D - Node[" << mac_->getMacNodeId() << ", User[" << senderId << ", Codeword[ " << cw << "], ACID[" << (unsigned int)acid << "] " << endl;
        ...
        {
            // exit if the current active HARQ process is not ready for retransmission
            EV << NOW << " LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D User is on ACID " << (unsigned int)currentAcid << " HARQ process is IDLE. No RTX scheduled ." << endl;
        ...
        for (unsigned int i = 0; (i < size) && (!finish); ++i)
        {
            ...
            EV << NOW << " LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D BAND " << b << endl;
            EV << NOW << " LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D total bytes:" << bytes << " still to serve: " << toServe << " bytes" << endl;
            EV << NOW << " LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D Available: " << bandAvailableBytes << " bytes" << endl;
            ...
            else
            {
                ...
                // signal end loop - all data have been serviced
                finish = true;
                EV << NOW << " LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D ALL DATA HAVE BEEN SERVICED"<< endl;
            }
        ...
        {
            // process couldn't be served - no sufficient space on available bands
            EV << NOW << " LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D Unavailable space for serving node " << senderId << " ,HARQ Process " << (unsigned int)currentAcid << " on codeword " << cw << endl;
        ...
        else
        {
            ...
            for(unsigned int i = 0; i < size; ++i)
            {
                ...
                if (allocatedCw!=MAX_CODEWORDS)
                {
                    EV << NOW << " LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D - adding " << assignedBlocks.at(i) << " to band " << i << endl;
            ...
            EV << NOW << " LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D HARQ Process " << (unsigned int)currentAcid << " : " << bytes << " bytes served! " << endl;
    ...
    {
        throw cRuntimeError("Exception in LteSchedulerUeAutoD2DUl::schedulePerAcidRtxD2D(): %s", e.what());
    }
    delete(bandLim);
    return 0;
}

void LteSchedulerUeAutoD2DUl::initHarqStatus(MacNodeId id, unsigned char acid)
{
    EV << NOW << ")LteSchedulerUeAutoD2DUl::initHarqStatus - initializing harq status for id " << id << "to status " << (unsigned int)acid << endl;
    harqStatus_[id]=acid;
}

void LteSchedulerUeAutoD2DUl::removePendingRac(MacNodeId nodeId)
{
