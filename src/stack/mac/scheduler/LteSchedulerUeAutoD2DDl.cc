...
bool
LteSchedulerUeAutoD2DDl::checkEligibility(MacNodeId id, Codeword& cw)
{
    ...
    if (mac_->getHarqTxBuffers()->find(id) != mac_->getHarqTxBuffers()->end())
    {
        ...
        if (freeUnits.first != HARQ_NONE)
        {
            ...
            // DEBUG check
            if (cw > MAX_CODEWORDS)
                throw cRuntimeError("LteSchedulerUeAutoD2DDl::checkEligibility(): abnormal codeword id %d", (int) cw);
            return true;
        }
    }
    return true;
...
unsigned int
LteSchedulerUeAutoD2DDl::schedulePerAcidRtx(MacNodeId nodeId, Codeword cw, unsigned char acid,
    std::vector<BandLimit>* bandLim, Remote antenna, bool limitBl)
{
    ...
    EV << NOW << "LteSchedulerUeAutoD2DDl::rtxAcid - Node [" << mac_->getMacNodeId() << "], User[" << nodeId << "],  Codeword [" << cw << "]  of [" << codewords << "] , ACID [" << (int)acid << "] " << endl;
...
//    }
    ...
    for (unsigned int i = 0; i < size; ++i)
    {
        ...
        EV << NOW << "LteSchedulerUeAutoD2DDl::rtxAcid ----- BAND " << b << "-----" << endl;
        EV << NOW << "LteSchedulerUeAutoD2DDl::rtxAcid To serve: " << bytes << " bytes" << endl;
        EV << NOW << "LteSchedulerUeAutoD2DDl::rtxAcid Available: " << available << " bytes" << endl;
        ...
        if ((allocatedCw == 0))
        {
            ...
            EV << NOW << "LteSchedulerUeAutoD2DDl::rtxAcid Assigned blocks: " << blocks << "  blocks" << endl;
    ...
    if (bytes > 0)
    {
        ...
        EV << NOW << "LteSchedulerUeAutoD2DDl::rtxAcid Cannot serve HARQ Process" << acid << endl;
        return 0;
    }
    ...
    EV << NOW << " LteSchedulerUeAutoD2DDl::rtxAcid HARQ Process " << (int)acid << "  codeword  " << cw << " marking for retransmission " << endl;
    ...
    if (allocatedCw != 0)
    {
        ...
        --codewords;
        if (codewords <= 0)
            throw cRuntimeError("LteSchedulerUeAutoD2DDl::rtxAcid(): erroneus codeword count %d", codewords);
    }
    ...
    EV << NOW << " LteSchedulerUeAutoD2DDl::rtxAcid HARQ Process " << (int)acid << "  codeword  " << cw << ", " << bytes << " bytes served!" << endl;

    return bytes;
}

bool
LteSchedulerUeAutoD2DDl::rtxschedule()
{
    EV << NOW << " LteSchedulerUeAutoD2DDl::rtxschedule --------------------::[ START RTX-SCHEDULE ]::--------------------" << endl;
    EV << NOW << " LteSchedulerUeAutoD2DDl::rtxschedule Cell:  " << mac_->getMacCellId() << endl;
    EV << NOW << " LteSchedulerUeAutoD2DDl::rtxschedule Direction: " << (direction_ == DL ? "DL" : "UL") << endl;
    ...
    for(; it != et; ++it)
    {
        ...
        EV << NOW << " LteSchedulerUeAutoD2DDl::rtxschedule  UE: " << nodeId << endl;
        EV << NOW << " LteSchedulerUeAutoD2DDl::rtxschedule Number of codewords: " << codewords << endl;
        ...
        for(; jt != jet; ++jt,++process)
        {
            ...
            for (Codeword cw = 0; cw < codewords; ++cw)
            {
                if (allocatedCws_[nodeId]==codewords)
                break;
                EV << NOW << " LteSchedulerUeAutoD2DDl::rtxschedule process " << process << endl;
                EV << NOW << " LteSchedulerUeAutoD2DDl::rtxschedule ------- CODEWORD " << cw << endl;

                // skip processes which are not in rtx status
                if (jt->at(cw).second != TXHARQ_PDU_BUFFERED)
                {
                    EV << NOW << " LteSchedulerUeAutoD2DDl::rtxschedule detected Acid: " << jt->at(cw).first << " in status " << jt->at(cw).second << endl;

                    continue;
                }

                EV << NOW << " LteSchedulerUeAutoD2DDl::rtxschedule " << endl;
                EV << NOW << " LteSchedulerUeAutoD2DDl::rtxschedule detected RTX Acid: " << jt->at(cw).first << endl;
                ...
                if(bytes > 0)
                {
                    EV << NOW << " LteSchedulerUeAutoD2DDl::rtxschedule CODEWORD IS NOW BUSY!!!" << endl;
    ...
    EV << " LteSchedulerUeAutoD2DDl::rtxschedule OFDM Space: " << availableBlocks << endl;
    EV << "    LteSchedulerUeAutoD2DDl::rtxschedule --------------------::[  END RTX-SCHEDULE  ]::-------------------- " << endl;

    return (availableBlocks == 0);
}

unsigned int LteSchedulerUeAutoD2DDl::scheduleGrant(MacCid cid, unsigned int bytes, bool& terminate, bool& active, bool& eligible,
    std::vector<BandLimit>* bandLim, Remote antenna, bool limitBl)
{
    return LteSchedulerUeAutoD2D::scheduleGrant(cid, bytes, terminate, active, eligible, bandLim, antenna, limitBl);
}
