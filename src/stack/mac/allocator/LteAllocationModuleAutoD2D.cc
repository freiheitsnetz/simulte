...
LteAllocationModuleAutoD2D::LteAllocationModuleAutoD2D(LteMacUeAutoD2D *mac, Direction direction)
{
    mac_ = mac;
    dir_ = direction;
    bands_ = 0;
...
void LteAllocationModuleAutoD2D::initAndReset(const unsigned int resourceBlocks, const unsigned int bands)
{
...
void LteAllocationModuleAutoD2D::configureOFDMplane(const Plane plane)
{
...
void LteAllocationModuleAutoD2D::setRemoteAntenna(const Plane plane, const Remote antenna)
{
    /**
...
bool LteAllocationModuleAutoD2D::configureMuMimoPeering(const MacNodeId nodeId, const MacNodeId peer)
{
...
Plane LteAllocationModuleAutoD2D::getOFDMPlane(const MacNodeId nodeId)
{
    return (allocatedRbsUe_[nodeId].secondaryUser_) ? MU_MIMO_PLANE : MAIN_PLANE;
}

MacNodeId LteAllocationModuleAutoD2D::getMuMimoPeer(const MacNodeId nodeId) const
    {
...
unsigned int LteAllocationModuleAutoD2D::computeTotalRbs()
{
    ...
    // DEBUG
    EV << NOW << " LteAllocationModuleAutoD2D::computeTotalRbs " << dirToA(dir_) << " - total " << resourceBlocks <<
    ", allocated " << allocatedBlocks << ", " << availableBlocks << " blocks available" << endl;

    return ((unsigned int) availableBlocks);
}

unsigned int LteAllocationModuleAutoD2D::availableBlocks(const MacNodeId nodeId, const Remote antenna, const Band band)
{
...
unsigned int LteAllocationModuleAutoD2D::getAllocatedBlocks(Plane plane, const Remote antenna, const Band band)
{
    return allocatedRbsPerBand_[plane][antenna][band].allocated_;
}

unsigned int LteAllocationModuleAutoD2D::getInterferringBlocks(Plane plane, const Remote antenna, const Band band)
{
...
unsigned int LteAllocationModuleAutoD2D::availableBlocks(const MacNodeId nodeId, const Plane plane, const Band band)
{
...
bool LteAllocationModuleAutoD2D::addBlocks(const Band band, const MacNodeId nodeId, const unsigned int blocks,
...
bool LteAllocationModuleAutoD2D::addBlocks(const Remote antenna, const Band band, const MacNodeId nodeId,
...
unsigned int LteAllocationModuleAutoD2D::removeBlocks(const Remote antenna, const Band band, const MacNodeId nodeId)
{
    // Check if the band exists
    if (band >= bands_)
    {
...
unsigned int
LteAllocationModuleAutoD2D::rbOccupation(const MacNodeId nodeId, RbMap& rbMap)
{
