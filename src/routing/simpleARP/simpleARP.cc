...
simsignal_t simpleARP::sentReqSignal = registerSignal("sentReq");
simsignal_t simpleARP::sentReplySignal = registerSignal("sentReply");
...
static std::ostream& operator<<(std::ostream& out, const simpleARP::ARPCacheEntry& e)
{
    if (e.pending)
...
void simpleARP::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        binder_=getModuleFromPar<LteBinder>(par("binder"),this);

        isUp = isNodeUp();
    }
}

void simpleARP::finish()
{
}
...
void simpleARP::handleMessage(cMessage *msg)
{
    if (!isUp) {
...
void simpleARP::handleMessageWhenDown(cMessage *msg)
{
    if (msg->isSelfMessage())
...
bool simpleARP::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
bool simpleARP::isNodeUp()
{
    NodeStatus *nodeStatus = dynamic_cast<NodeStatus *>(findContainingNode(this)->getSubmodule("status"));
    return !nodeStatus || nodeStatus->getState() == NodeStatus::UP;
}

void simpleARP::updateDisplayString()
{
    std::stringstream os;
...
void simpleARP::initiateARPResolution(ARPCacheEntry *entry)
{
    ...
    // start timer
    cMessage *msg = entry->timer = new cMessage("simpleARP timeout");
...
void simpleARP::sendPacketToNIC(cMessage *msg, const InterfaceEntry *ie, const MACAddress& macAddress, int etherType)
{
...
void simpleARP::sendARPRequest(const InterfaceEntry *ie, IPv4Address ipAddress)
{
    ...
    // fill out everything in simpleARP Request packet except dest MAC address
...
void simpleARP::requestTimedOut(cMessage *selfmsg)
{
    ...
    if (entry->numRetries < retryCount) {
        // retry
        IPv4Address nextHopAddr = entry->myIter->first;
        EV_INFO << "simpleARP request for " << nextHopAddr << " timed out, resending\n";
        sendARPRequest(entry->ie, nextHopAddr);
    ...
    // max retry count reached: simpleARP failure.
    // throw out entry from cache
    EV << "simpleARP timeout, max retry count " << retryCount << " for " << entry->myIter->first << " reached.\n";
...
bool simpleARP::addressRecognized(IPv4Address destAddr, InterfaceEntry *ie)
{
    if (rt->isLocalAddress(destAddr)) {
        return true;
    }
    else if (respondToProxyARP) {
        // respond to Proxy simpleARP request: if we can route this packet (and the
...
void simpleARP::dumpARPPacket(ARPPacket *arp)
{
...
void simpleARP::processARPPacket(ARPPacket *arp)
{
    ...
    if (srcMACAddress.isUnspecified())
        throw cRuntimeError("wrong simpleARP packet: source MAC address is empty");
    if (srcIPAddress.isUnspecified())
        throw cRuntimeError("wrong simpleARP packet: source IPv4 address is empty");
    ...
    // "?Am I the target protocol address?"
    // if Proxy simpleARP is enabled, we also have to reply if we're a router to the dest IPv4 address
    if (addressRecognized(arp->getDestIPAddress(), ie)) {
        ...
        switch (arp->getOpcode()) {
            case ARP_REQUEST: {
                EV_DETAIL << "Packet was simpleARP REQUEST, sending REPLY\n";
            ...
            default:
                throw cRuntimeError("Unsupported opcode %d in received simpleARP packet", arp->getOpcode());
        }
    }
    else {
        // address not recognized
        EV_INFO << "IPv4 address " << arp->getDestIPAddress() << " not recognized, dropping simpleARP packet\n";
        delete arp;
    }
}

void simpleARP::updateARPCache(ARPCacheEntry *entry, const MACAddress& macAddress)
{
    EV_DETAIL << "Updating simpleARP cache entry: " << entry->myIter->first << " <--> " << macAddress << "\n";
...
MACAddress simpleARP::resolveL3Address(const L3Address& address, const InterfaceEntry *ie)
{
    IPv4Address tmp=address.toIPv4();
    return binder_->getMACfromIP(tmp);
}

L3Address simpleARP::getL3AddressFor(const MACAddress& macAddr) const
{
    Enter_Method_Silent();
